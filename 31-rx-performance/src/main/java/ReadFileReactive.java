...
public class ReadFileReactive {
    ...
    private static List<String> FILE_CONTENT = null;

    private synchronized static List<String> getFileContent() {
        if (FILE_CONTENT == null) {
            try (BufferedReader reader = new BufferedReader(new FileReader("prices.csv"))) {
                FILE_CONTENT = reader.lines().collect(Collectors.toList());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
	...
	public static void main(String[] args) throws IOException {
		...
		doReadFile(DEV_NULL)
            .then(Mono.fromRunnable(() -> doRun(DEV_NULL, 1000)))
            .subscribe();
		System.in.read();
	}
	...
	private static void doRun(Consumer<String> handle, int n, long runStart) {
        List<Mono<Long>> runs = new ArrayList<>(n);
		for(int i = 0; i < n; i ++) {
            ...
            runs.add(
                    doReadFile(handle)
                        ...
                        .then(Mono.fromCallable(() -> System.nanoTime() - start))
            );
		}

        Flux.merge(runs).collectList().subscribe(list -> {
            list.forEach(results::add);
            doFinish(n, System.nanoTime() - runStart);
        });
	}

	private static Flux<String> doReadFile(Consumer<String> cons) {
		return Flux.fromIterable(getFileContent())
