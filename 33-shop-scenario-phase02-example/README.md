...
- **Start all required containers** for development: `docker compose up -d kafka init-kafka mongodb`
- **Start all containers** for development: `docker compose up -d`
- Start only Kafka: `docker compose up -d kafka`
...
- Run as Java Application by using `Application.java` for **Shopping Service**, **Checkout Service** and maybe **Payment Service**
...
_Requires to use docker compose up -d mongo-express kui_
...
- Apply Reactive Programming in the **Shopping Service**, **Checkout Service**, **Payment Service** or **Warehouse Service**
- Take a look at the following components of the implementation:
  - `MessageProducerService`: Integration with Kafka for distributing messages and events
  - `...ConsumerService`: Integration with Kafka for polling messages and events
  - `...Service`: Standard services for business logic
  - `...Controller`: REST controller for client integration
- Apply Reactive Programming to modify the **Warehouse Adapter** as well

_Remarks:_

- _Take a look at previous WebFlux examples if needed_
- _Spring Boot applications have a class with a main method annotated with `@SpringBootApplication`_
- _Annotate any class with `@Service`, `@RestController` or `@Component` — Spring will instantiate it_
- _Use `@Autowired` on fields or constructor parameters to realize Dependency Injection: instances are assigned automatically_
- _Methods annotated with `@PostConstruct` and `@PreDestroy` are executed after creation and before destruction, ideal for setup and tear-down_
- _Dependencies for MQTT client and Kafka client are already available_
- _Serialize any object to a JSON string with `new ObjectMapper().writeValueAsString(obj);`_
- _Deserialize a string into a class with `SomeMessageClass message = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).readValue(value, SomeMessageClass.class);`_
- _`CartMessage` in `CartMessageConsumerService` in the checkout service is configured so that the ObjectMapper deserializes the correct subclass (side note)_
- _You will find a starter service in the folder that you can use as a blank slate_
